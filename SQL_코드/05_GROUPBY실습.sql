...
-- 그룹을 세분화하여 조회하기 --
--1. 각 부서별 사원들의 퇴사여부를 조회하고 사원수 조회
...
--4. 각 직급별 최소 급여가 300만원 이하인 직급 조회
--MIN AS "최소급여"
...
--5. EMPLOYEE 테이블에서 부서별 70년대생의 급여 평균이 300만 이상인 부서를 조회하고 부서코드 오름차순 정렬
SELECT DEPT_CODE, FLOOR(AVG(SALARY)) AS "급여 평균"
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 1, 1) = '7'
--WHERE SUBSTR(EMP_NO, 1, 2) BETWEEN 70 AND 79 : 가능!
--WHERE TO_NUMBER(SUBSTR(EMP_NO, 1, 2)) BETWEEN 70 AND 79 : 가능!
GROUP BY DEPT_CODE
...
--3. 부서별, 직급별 급여 평균
--DEPT_CODE, JOB_CODE, ROUND(AVG(SALARY))
--DEPT_CODE 오름차순
SELECT DEPT_CODE, JOB_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE, JOB_CODE)
ORDER BY DEPT_CODE;
...
--DEPT_CODE 오름차순
SELECT DEPT_CODE, JOB_CODE, COUNT(*) AS "직원 수"
FROM EMPLOYEE
